...
with open ('test.txt','r') as file_read:
    for i in file_read:  #reading each line of text file 
        if (r>=1):      #logic to create new sheet if specified no of rows are over in current sheet
            sheet=sheet+1
        ...
        if (sheet>1 and r==0):  #inserting header in new worksheet
            for k in header_list:
                value=k.rstrip('\n')
                worksheet.write(r,c,value,bold)
                c=c+1
            r=r+1
            c=0
        for k in column_list: # inserting columns values from list into each column of a worksheet row
            value=k.rstrip('\n')
            if (r==0):
                worksheet.write(r,c,value,bold)
            else:
